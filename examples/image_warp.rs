...
fn run() {
  ...
  let mut warped_img = [[[0u8; 3]; IMG_WIDTH]; IMG_HEIGHT];
  let mut composite_img = [[[0u8; 3]; IMG_WIDTH]; IMG_HEIGHT];

  let start = SystemTime::now();
  for y in 0..IMG_HEIGHT {
    let path = DtwSolver::<[u8; 3], IMG_WIDTH, MAX_PATH_SIZE>::new(
      &left_img[y],
      &right_img[y],
      downsample_fns::mean,
...
fn load_image<const H: usize, const W: usize>(filepath: &'static str) -> [[[u8; 3]; W]; H] {
  let mut img = [[[0u8; 3]; W]; H];
  ...
  for y in 0..H {
    for x in 0..W {
      ...
      img[y][x] = [pixel[0], pixel[1], pixel[2]];
    }
  }
...
fn save_image<const H: usize, const W: usize>(filepath: &'static str, img: [[[u8; 3]; W]; H]) {
  ...
  for (img_x, img_y, pixel) in img_buf.enumerate_pixels_mut() {
    ...
    *pixel = image::Rgb([img[y][x][0], img[y][x][1], img[y][x][2]]);
  }
